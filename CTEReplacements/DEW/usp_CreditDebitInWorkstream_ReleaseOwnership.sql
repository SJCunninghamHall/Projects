CREATE PROCEDURE [DataAccess].[usp_CreditDebitInWorkstream_ReleaseOwnership] @UserName VARCHAR(30)
AS
BEGIN
    SET NOCOUNT ON;
    ...
    BEGIN TRY
        ...
        SELECT @UserId = [UserSeq]
        FROM [DataAccess].[User]
        WHERE [UserName] = @UserName;

        SELECT @End = [WorkstreamStateSeq]
        FROM [CFG].[WorkstreamState]
        WHERE [Name] = 'End'

		INSERT INTO @DebitInWorkStreamAudit
        (
		...
		SELECT	CAST(ROUND([DebitInWorkstreamSeq] / 100000000000, 0) * 100000000000 AS BIGINT) + CAST(NEXT VALUE FOR [Process].[sqn_DebitInWorkstreamAudit] AS BIGINT),
				[DebitInWorkstreamSeq],
				[StateSeq],
				NULL,
				GETDATE()
        FROM	[Process].[DebitInWorkstream] 
        WHERE	[StateSeq] <> @End
		...
		WHERE	
			[StateSeq] <> @End
		AND 
			[GroupSeq] IS NOT NULL
		AND 
			[UserSeq] = @UserId
		AND 
			[UserDecisionSeq] IS NULL
		AND 
		(
			[UserSuspended] IS NULL
		OR 
		...
		CREATE CLUSTERED INDEX ci_DebitInWorkstreamSeq ON #ReleaseOwnershiprecord(DebitInWorkstreamSeq)
        ...
        SET		
			...
			[UserSeq] = NULL
        FROM	
			[Process].[DebitInWorkstream] [DIW]
		INNER JOIN 
			#ReleaseOwnershiprecord [RO]
		ON 
			[RO].[DebitInWorkstreamSeq] = [DIW].[DebitInWorkstreamSeq];

		--Capture the Audit log
		EXEC [Process].[usp_06MD_Insert_DebitInWorkstreamAudit] @DebitInWorkStreamAudit;
    ...
    BEGIN CATCH
        ...
        EXECUTE [Base].[usp_LogException] @ErrorMessageLog;
        THROW;
    END CATCH;
END;
...
', @level0type = N'SCHEMA', @level0name = N'DataAccess', @level1type = N'PROCEDURE', @level1name = N'usp_CreditDebitInWorkstream_ReleaseOwnership';
