			...
			(	
				...
				WHERE 
					(
						...
						AND 
							FD.ResponseDate IS NULL
						) 
					OR 
						FD.ResponseDate = @BusinessDate
					) 
				AND 
					(
						NPR.APGReasoncodeText IS NULL 
					OR 
						NPR.APGReasoncode IS NULL
					)
				AND 
					( --Get only no pay items
						FD.PayReason IS NOT NULL
					OR 
						I.cf_NoPaySuspectRsn IS NOT NULL
					)
				) AS ART
				...
				WHERE 
					...
					OR 
						( 
							FD.PayDecision = 0
						AND 
							FD.ResponseDate = @BusinessDate -- On Day 3 Get MSG13 which had local bank holiday's
						) 
					)
				AND 
					NPR.Industrycode IS NULL 
			) AS IND

		IF  (@APGResonTxt  <> '' OR @NPASortcode <> '' OR @IndustryCode <> ''  )
		BEGIN 	
...
/* 		;WITH ItemDetails 
				...
				SELECT
					...
					,CASE 
						...
						THEN
							( 
								CASE 
									WHEN NPR.APGReasoncodeText = 'Adjusted' AND FD.AdjustmentReason = 90
									THEN 'A020'
		...
		CREATE CLUSTERED INDEX ci_EntityState ON #ItemDetails(EntityState, ResponseDate)
        ...
        BEGIN CATCH
            ...
            EXECUTE [Base].[usp_LogError] @ErrorMessage;
			THROW ;
        END CATCH;
    END;
GO
GRANT EXECUTE ON [MMP].[usp_GetPostingDetails] TO [MMPLoader];
GO

EXECUTE sp_addextendedproperty @name = N'Component', @value = N'MMP',
    @level0type = N'SCHEMA', @level0name = N'MMP', @level1type = N'PROCEDURE',
