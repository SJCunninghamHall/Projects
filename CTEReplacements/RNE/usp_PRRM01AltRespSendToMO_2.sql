  ...
  SET NOCOUNT ON;
	...
	BEGIN TRY
			...
			BEGIN
					...
					@PostingResponseRecord =
					   (
							...
							ORDER BY 
								[ResponseSequence]  
							FOR 
								XML PATH('PostingResponseRecord') , TYPE
						)
					...
					@EntityDetails = 
						(
							SELECT 
								...
								CASE 
									WHEN EntityType = 'I' AND DATALENGTH(EntityIdentifier) = 25 
									THEN EntityIdentifier
									WHEN EntityType = 'D' AND DATALENGTH(EntityIdentifier) = 25 
									THEN EntityIdentifier
									WHEN EntityType = 'T' AND DATALENGTH(EntityIdentifier) = 25 
									THEN EntityIdentifier
									ELSE '.....'	 
								END  AS "EntityId",  
				...
				INSERT INTO 
					[Posting].[RNEMOQueueDetails]([RNEMOID],
					BusinessDate,
					ExtractId,
					PostingID,
...
END;
GO
EXECUTE sp_addextendedproperty @name = N'Version', @value = N'1.0.1', @level0type = N'SCHEMA', @level0name = N'Posting', @level1type = N'PROCEDURE', @level1name = N'usp_PRRM01AltRespSendToMO';


GO
EXECUTE sp_addextendedproperty @name = N'MS_Description', @value = N'Sends the alternate response XML back to MO', @level0type = N'SCHEMA', @level0name = N'Posting', @level1type = N'PROCEDURE', @level1name = N'usp_PRRM01AltRespSendToMO';


GO
EXECUTE sp_addextendedproperty @name = N'Component', @value = N'iPSL.ICE.RNE.Database', @level0type = N'SCHEMA', @level0name = N'Posting', @level1type = N'PROCEDURE', @level1name = N'usp_PRRM01AltRespSendToMO';
