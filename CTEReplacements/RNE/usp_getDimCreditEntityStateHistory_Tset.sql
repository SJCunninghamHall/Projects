...
SET NOCOUNT ON;
	...
	BEGIN
		...
		LEFT JOIN
			Base.vw_Document		AS DocD4
		ON
			DocD4.DocumentId = TXT.DocumentId
		LEFT JOIN
			@Credit_RejectedItems	AS Rej
		ON
		...
		AND En.EntityType = 'T'
		AND Rej.CreditId IS NULL
		AND MsgType.MessageType NOT IN ( 'MSG15',
										'MSG16'
									);
...
GRANT
	EXECUTE
ON [RNEReport].[usp_getDimCreditEntityStateHistory_Tset]
TO
	[RNEReportAccess];
