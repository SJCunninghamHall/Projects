CREATE PROCEDURE[RNEReport].[usp_getDimCreditEntityStateHistory_Tset] 
	@PreviousBusinessDateRangeStart BIGINT
...
BEGIN
	...
	-- Get Rejected Credits
	DECLARE @Credit_RejectedItems [RNEReport].[tv_RejectedCreditInformation]

	INSERT INTO 
		@Credit_RejectedItems
	EXECUTE [RNEReport].[csp_getCreditRejectedItems] 
			@BusinessDateRangeEnd
			,@PreviousBusinessDateRangeStart  


	SELECT
		EntityType AS EntityType
		,TXT.TransactionSetIdWithVersion AS TransactionSetIdWithVersion
		,DocD4.DocumentMessageId AS DocumentMessageId
		,CrT.CreditId AS EntityIdentifier
		,EntityId AS EntityId
		,en.Revision  AS Revision
		,en.EntityState  AS EntityState
		,CAST(
				CASE 
					WHEN ISNUMERIC(RIGHT(Co.IntMessageType,2)) = 1 
					THEN RIGHT(Co.IntMessageType,2)
				ELSE 
					RIGHT(Co.IntMessageType,1)
				END AS TINYINT) 
	...
	LEFT JOIN 
		Base.Credit CrT 
	ON 
		TXT.InternalTxId = CrT.InternalTxId
	LEFT JOIN 
		Base.vw_Document DocD4 
	ON 
		DocD4.DocumentId = TXT.DocumentId
	LEFT JOIN 
		@Credit_RejectedItems Rej 
	ON 
		Rej.CreditId=En.EntityIdentifier							
	WHERE   
		En.EntityId BETWEEN @BusinessDateRangeStart AND @BusinessDateRangeEnd
	AND 
		En.EntityType = 'T'  
	AND 
		Rej.CreditId IS NULL 
	AND 
		MsgType.MessageType NOT IN ('MSG15','MSG16')
	...
	SELECT	
		...
		--,FD.AdjustmentReason AS AdjustmentReason
		,CASE 
		...
		(
			...
			FROM 
				 (
					...
					WHERE 
						msg.MessageType  IN ('MSG01','MSG06') AND 
					(
						CASE 
							WHEN EntityType = 'I' 
...
GRANT Execute ON [RNEReport].[usp_getDimCreditEntityStateHistory_Tset] to [RNEReportAccess]
...
EXECUTE sp_addextendedproperty @name = N'Version', @value = N'$(Version)',
    ...
    @level1name = N'usp_getDimCreditEntityStateHistory_Tset';
GO
