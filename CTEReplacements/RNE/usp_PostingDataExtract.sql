...
AS
    ...
    SELECT  @SchemaVersion = ( SELECT   [ConfigValue]
                               FROM     [Config].[ApplicationConfig]
                               WHERE    ConfigType = 'PostingRequest'
                                        AND [ConfigParameter] = 'SchemaVersion'
                             ) ,
            @Participant = ( SELECT [ConfigValue]
                             FROM   [Config].[ApplicationConfig]
                             WHERE  ConfigType = 'PostingRequest'
                                    AND [ConfigParameter] = 'ParticipantID'
                           ) ,
            @ProcessingDate = CONVERT(VARCHAR(19), ( SELECT CAST(BusinessDate AS DATE)
                                                     FROM   [Config].[ProcessingDate]
                                                   ), 126) ,
            @FileDate = CONVERT(VARCHAR(19), GETUTCDATE(), 126) ,
            @FileSequenceNumber = ( SELECT  Posting.sfn_pad_string(MOSequence,
                                                              '0', 4, 0)
                                    FROM    Posting.RNEMOQueueDetails
                                    WHERE   RNEMOID = @RnEMoID
                                  ) ,
            @FileVersion = 1 ,
            @Weekday = UPPER(LEFT(FORMAT(( SELECT   CAST(BusinessDate AS DATE)
                                           FROM     [Config].[ProcessingDate]
                                         ), 'ddd'), 2)) ,
			@BusinessDate =( SELECT CONVERT(VARCHAR(10),BusinessDate,121)
                                           FROM     [Config].[ProcessingDate]
								) ,
			@NextProcessingDate =( SELECT CONVERT(VARCHAR(10),NextDate,121)
                                           FROM     [Config].[ProcessingDate]
								) ,
            @Environment = ( SELECT [ConfigValue]
                             FROM   [Config].[ApplicationConfig]
                             WHERE  ConfigType = 'PostingRequest'
                                    AND [ConfigParameter] = 'Environment'
                           ) ,
            @Currency = ( SELECT    [ConfigValue]
                          FROM      [Config].[ApplicationConfig]
                          WHERE     ConfigType = 'PostingRequest'
                                    AND [ConfigParameter] = 'Currency'
                        ) ,
			@CaptureDateFormat= ( SELECT DtFormat
									    FROM [Config].[PostingCustomDateFormatting]
									   WHERE ColumnName = 'CaptureDate'
								    ),
			@NPAChannel = ( SELECT    [ConfigValue]
                          FROM      [Config].[ApplicationConfig]
                          WHERE     ConfigType = 'PostingRequest'
                                    AND [ConfigParameter] = 'NPAChannel'
                        ),
		    @JGAccountTranCodes = ( SELECT    [ConfigValue]
                          FROM      [Config].[ApplicationConfig]
                          WHERE     ConfigType = 'JGAccount'
                                    AND ConfigParameter = 'EligibleJGAccountTranCodes'
                        ),
			 @AggITP = ( SELECT [ConfigValue]
                             FROM   [Config].[ApplicationConfig]
                             WHERE  ConfigType = 'PostingRequest'
                                    AND [ConfigParameter] = 'AggITP'
                        ),
			 @RCOPattern9SortCode = ( SELECT [ConfigValue]
                             FROM   [Config].[ApplicationConfig]
                             WHERE	ConfigType = 'RCOPattern9'
									AND [ConfigParameter] = 'SortCode'
                        ),
			 @RCOPattern9AccountNumber = ( SELECT [ConfigValue]
                             FROM   [Config].[ApplicationConfig]
                             WHERE	ConfigType = 'RCOPattern9'
    ...
    BEGIN TRY
        ...
        BEGIN
			...
			SELECT
				...
				C.Sortcode,
				--C.AccountNumber,
				IIF(((ISNULL(LTRIM(RTRIM(c.TranCode)),LTRIM(RTRIM(c.TranCode_New)))) IN (SELECT [value] FROM string_split(@JGAccountTranCodes,',')) AND C.[JGAccount] IS NOT NULL ),('0' + Right(C.[JGAccount],7)),C.AccountNumber ) AS 'AccountNumber',
				C.Reference,
				C.Amount,
				...
				TransactionSetId,
				EntityState,
				ROW_NUMBER() OVER ( PARTITION BY CreditId,TransactionSetId  ORDER BY  InternalId DESC ) CrRank
			INTO
				#Credits
			FROM 
				Outclearing.Credit C
			WHERE 
				PostingID = @PostingID

			CREATE NONCLUSTERED INDEX 
				nci_TransactionSetId 
			ON 
				#Credits(TransactionSetId)
			WHERE 
				CrRank = 1
			...
			SELECT			
				...
				D.ItemType,
				D.EntityState,
				ROW_NUMBER() OVER ( PARTITION BY Cr.CreditId,D.DebitId,D.TransactionSetId  ORDER BY  D.InternalId DESC ) DrRank
			INTO
				#CreditsDebits
			FROM  
				#Credits Cr
			...
			CREATE NONCLUSTERED INDEX 
				nci_CI_DI_DR 
			ON 
				#CreditsDebits(CreditId, DebitId, DrRank)
			WHERE
				DrRank = 1
			...
			CREATE NONCLUSTERED INDEX 
				nci_TransactionSetId 
			ON 
				#CreditsDebits(TransactionSetId)
			...
			SELECT   
				...
				EntityIdentifier =
				CASE 
					WHEN @AggITP=0 AND PP.IsMultiCrdts=1 AND PP.PatternID=1 
					THEN crdr.CreditID
					ELSE PP.ItemIdentifier
				END,
				NULL PostingIdentifier ,
				ItemIdentifier =
				CASE 
					WHEN @AggITP=0 AND PP.IsMultiCrdts=1 AND PP.PatternID=1 
					THEN crdr.CreditID
				...
				END,
				RIGHT(CONCAT(REPLICATE('0', 6),ROW_NUMBER() OVER (ORDER BY @PostingID )), 6) AS PostingSeq,
				PP.PostingType ,
				'Outclearing' AS PostingSubType ,
				CASE	
					WHEN  (PP.PostingType = 'REJ' AND Tx.EntityState = 130) 
					THEN 'MSG03' 
					WHEN  (PP.PostingType = 'REJ' AND Tx.EntityState IN (30,60,90)) 
					THEN 'MSG01'
					ELSE 'MSG01' 
				END AS SourceMsg ,
				Channel = 
					CASE  
						WHEN PP.PostingType IN ( 'NPA') AND @NPAChannel IS NOT NULL THEN @NPAChannel
						ELSE CONVERT(VARCHAR(4),Tx.AltSource) 
					END ,
				CredDebInd = 
					CASE 
						WHEN PP.PostingType IN ('ITP','REJ') 
						THEN 'Debit'
						WHEN (PP.IsPostingOnCredit=0 AND PP.PostingType = 'NPA') 
						THEN 'Debit'
						ELSE 'Credit'
					END,
				PostSource = 
				...
				IIF(@AggITP=0 AND PP.PostingType = 'ITP' AND PP.PatternID=1 AND Tx.IsMultiCrdts=1 ,crdr.OriginalAmount_ISO_Cr,PP.PostingAmount) AS Amount ,	--Fix given based on 194816 bug	
				crdr.TransactionSetID ,
				PP.[REJReasonCode] [RecordReason],
				...
				CASE	
					WHEN PP.PostingType IN ('REJ' ,'DCR')
				...
				CASE 
					WHEN (@AggITP=0 AND PP.PostingType = 'ITP' AND PP.PatternID=1)  --Fix given based on 203206 bug 
				...
				IIF(@AggITP=0 AND PP.PostingType = 'ITP' AND PP.IsMultiCrdts=1 AND PP.PatternID=1,crdr.OriginalAmount_ISO_Cr,crdr.Amount) AS DebitFullAmt ,		

				---------Credit Record --------------
				crdr.CreditId AS CreditTransactionID ,

				CASE 
					WHEN PP.PostingType = 'RCO' AND PatternID = 8--Fix given based on 196195 bug
					THEN crdr.Sortcode 
					WHEN PP.PostingType = 'RCO' AND PatternID = 9
			...
			FROM 
				Outclearing.vw_DerivedPostingType PP
			INNER JOIN 
				#CreditsDebits crdr 
			ON 
				crdr.CreditId = PP.CreditID 
			AND 
				crdr.DebitId = PP.DebitID 
			AND 
				crdr.DrRank = 1

			INNER JOIN 
				Outclearing.TXSet Tx 
			ON 
				PP.PostingID = Tx.PostingID 
			AND 
				Tx.TransactionSetIdWithVersion = PP.TransactionSetIdWithVersion	
			...
			AND 
				...
				AND 
					PP.PostingType NOT IN ('ERR','NIL')
				)
			AND 
				PP.[EligibleToMo] = 1
			AND 
				ISNULL(IsDay1Exception,0) = 0
...
GRANT EXECUTE ON [Outclearing].[usp_PostingDataExtract] TO [RNESVCAccess];
