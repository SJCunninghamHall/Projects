        ...
        BEGIN TRY
            ...
            BEGIN
				...
				INTO 
					#EligibleCreditItems
				FROM 
					[Report].[dimCreditEntityStateHistory] Cr 
				WHERE 
					DateKey = @Date 
				AND 
					EntityState IN (20,21) 
				AND 
					EntityType = 'I'
				GROUP BY 
				...
				INTO 
					#FinalCreditItems
				FROM 
					#EligibleCreditItems Cr 

				INNER JOIN 
					[Report].[factCreditAmounts] CrAmnt 
				ON 
					Cr.ICSITemID=CrAmnt.CreditId  

				INNER JOIN 
					[Report].dimTransactionSet Tx 
				ON 
					Tx.TransactionSetId=CrAmnt.TransactionSetKey

				INNER JOIN 
					config.Channel crSrc --Get items related to configured sources menioned in config.channel table i.e., 5000,5100 etc
				ON 
					crSrc.ChannelID=Tx.AltSource 
				AND 
					crSrc.ReportID=@ReportId 

				INNER JOIN 
					Config.ReportNoPayReason crAdj  -- Gets items that are matching the required adjustment codes
				ON 
					crAdj.NoPayReasonCode=cr.AdjustmentCode 
				AND 
					crAdj.ReportID=@ReportId
				AND 
					crAdj.Category='Adjustment'  

				INNER JOIN 
					Config.ReportNoPayReason crNP  -- Gets items that are matching the required adjustment codes
				ON 
					crNP.NoPayReason=cr.NoPayReason 
				AND 
					crNP.ReportID=@ReportId  
				AND 
					crNP.Category='NoPay' 

				INNER JOIN 
					Config.ReportBranchType RpBr  -- Gets Branch codes for the report
				ON 
					RpBr.ReportId=@ReportId 

				INNER JOIN 
					Config.[SortCodeBranchType] Br  -- Gets items that are matching the required adjustment codes
				ON 
					Br.OutclearingBranchType=RpBr.BranchCode
				AND 
					Br.SortCode=Cr.Sortcode
					...
					(
						...
						(
							SELECT 
								EntityState
							FROM 
								Report.dimReportEntityState IncSt
							WHERE 
								ReportID = @ReportId
							AND 
								ColumnName = 'InclusionState' 
							AND 
								ItemIndicator = 'Cr' 
							AND 
								EXISTS 
					...
					(
						...
						(
							SELECT 
								EntityState
							FROM 
								Report.dimReportEntityState ExSt
							WHERE 
								ReportID=@ReportId
							AND 
								ColumnName='ExclusionState' 
							AND 
								ItemIndicator='Cr' 
							AND 
								EXISTS 
				...
				CREATE NONCLUSTERED INDEX nci_TransactionSetId ON #FinalCreditItems(TransactionSetId)
				...
				FROM 
					#FinalCreditItems Cr
				INNER JOIN 
					[Report].[factDebitAmounts] DbAmnt 
				ON 
					Cr.TransactionSetId = DbAmnt.TransactionSetKey
				INNER JOIN 
					[Report].[dimDebitInformation] Db 
				ON 
...
GRANT EXECUTE ON [RnEWareHouseReports].[usp_CustomerAdviceReport] TO [RnESsrsDwAccess]
GO

EXEC sys.sp_addextendedproperty @name = N'Component',
    @value = N'RnEReportDataWarehouse', @level0type = N'SCHEMA',
    @level0name = N'RnEWareHouseReports', @level1type = N'PROCEDURE',
...
EXEC sys.sp_addextendedproperty @name = N'Version', @value = N'$(Version)',
    @level0type = N'SCHEMA', @level0name = N'RnEWareHouseReports', @level1type = N'PROCEDURE',
    @level1name = N'usp_CustomerAdviceReport';
GO
