...
AS
	BEGIN TRY
		...
		BEGIN
			...
			SELECT	DISTINCT
					TS.TransactionSetId AS TransactionSetKey,
			...
			ON #TransactionCTE (TransactionSetKey);
			...
			FROM
				(
					...
					FROM
						Report.dimDebitEntityStateHistory	AS DI
					INNER JOIN
						[Report].[factDebitAmounts]			AS FD
					ON
						FD.DebitId = DI.DebitId
					...
					AND DI.IntMessageType IN ( '06MA02' )
					AND DI.SettlementDate = @BusinessDate
					UNION ALL
					SELECT
...
GRANT
	EXECUTE
ON [WAR].[usp_WrongDeliveredReport]
TO
	[RnEWAR];
GO
