...
SET NOCOUNT ON;
	BEGIN
		...
		ON
			En.CoreId = Co.CoreId
		LEFT JOIN
			Base.vw_Document	AS DocD5
		ON
			DocD5.DocumentMessageId = En.EntityIdentifier
		LEFT JOIN
			Base.vw_TXSet		AS Tx5
		ON
		...
		CREATE NONCLUSTERED INDEX nci_ItemId_IntMessageType_Rnk
		ON #tempFrdResult (ItemId, IntMessageType, Rnk);
		...
		CREATE NONCLUSTERED INDEX nci_FCMIdentifier_MessageType_Rnk
		ON #Item (FCMIdentifier, MessageType, Rnk);
		...
		SELECT
			...
			CASE
				WHEN [ENT].[MessageType] IN ( 'MSG02',
												'MSG03'
											)
				THEN 'MSG01'
				ELSE [ENT].[MessageType]
		...
		CREATE NONCLUSTERED INDEX nci_MessageType_Derived
		ON #ItemsEntities (MessageType_Derived);
		CREATE NONCLUSTERED INDEX nci_EntityId
		ON #ItemsEntities (EntityId);
		...
		AND [I].[Rnk] = 1
		AND I.MessageType = [IMC].[MessageType_Derived]
		LEFT JOIN
		...
		AND fr.MessageType = [IMC].[MessageType_Derived]
		AND [fr].[Rnk] = 1
		LEFT JOIN
			(
				SELECT
...
GRANT
	EXECUTE
ON [RNEReport].[usp_getDimCreditEntityStateHistory_Item]
TO
	[RNEReportAccess];
