...
AS
	BEGIN TRY
		...
		BEGIN	
			...
			SET @ReportID 		= (SELECT ReportID FROM  Report.dimReportMaster WHERE ReportNo =@ReportNo);
			SET @BusinessDate  	= (SELECT [FullDate] FROM [Report].[dimDate] WHERE isCurrentBusinessDate =1);
			SET @Day2Date      	= (SELECT NextDate FROM [Report].[dimDate] WHERE isCurrentBusinessDate =1);
			...
			INNER JOIN
				Report.dimReportEntityState DE
			ON
				DI.MSG03 = DE.EntityState
			AND 
				DE.MessageType = 'MSG03'
			WHERE 
				DE.ReportID = @ReportID 
			AND 
				ColumnName ='RTPHoldover'
			
			CREATE NONCLUSTERED INDEX nci_DebitId ON #REJItems(DebitId)
			
			SELECT	
				DebitId AS "Item Reference",
				DebitEntryDate AS "Debit Received Date",
				Day2Date AS "RTP Response Receipt Due Date",
				...
				dp.ParticipantName "Brand or Agency"
			FROM
				(	
					SELECT DISTINCT
						DI.DebitId,
					...
					INNER JOIN 
						Report.dimCreditInformation dc 
					ON 
						fc.CreditId = dc.CreditId
					INNER JOIN 
						Report.dimParticipantData dp 
					ON 
					...
					ON 
						(
							...
							AND 
								DE.MessageType = 'MSG04'
							AND 
								DE.ReportID = @ReportID -- SJC common to both
							AND 
								ColumnName ='RTPHoldover' -- SJC common to both
							)
						OR
							(
								DI.MSG01 = DE.EntityState 
							AND 
								DE.MessageType = 'MSG01'
							AND 
								DE.ReportID = @ReportID -- SJC common to both
							AND 
								ColumnName ='RTPHoldover' -- SJC common to both
					...
					AND 
						dp.ONUSFlag = 1 
					AND 
						DI.DebitEntryDate = @BusinessDate
				) A 
			INNER JOIN 
				Report.dimParticipantData DP 
			ON 
...
GRANT EXECUTE ON [RnEWareHouseReports].[usp_BeneficiaryHoldoverReportCSV] to [RnESsrsDwAccess]
