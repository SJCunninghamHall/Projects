...
AS
	BEGIN
		...
		SELECT
			...
			ROW_NUMBER() OVER (PARTITION BY
								...
								CASE
									...
									AND En.EntityState = 960
									THEN 1
									WHEN ResponseStatus = 'Y'
									AND En.EntityState = 970
									THEN 2
									ELSE 9
								END
							)						AS RKD,
		...
		INTO
			#DebitItems
		FROM
			Post.ResponseDetail		AS RD
		INNER JOIN
			RNE.vw_FinalDebit_TSet	AS FD
		ON
			RD.ItemId = FD.DebitId
		INNER JOIN
			RNE.vw_FinalCredit_TSet AS FC
		ON
			FD.TransactionSetId = FC.TransactionSetId
		INNER JOIN
			Post.Response			AS resp
		ON
		...
		AND RD.ItemId = En1.EntityIdentifier
		LEFT JOIN
			Base.Entity				AS En
		ON
			resp.CoreId = En.CoreId
		AND FC.CreditId = En.EntityIdentifier
		WHERE
			RD.InternalId
...
GRANT
	EXECUTE
ON [RNEReport].[usp_getDimDebitPostingResponse]
TO
	[RNEReportAccess];
