		...
		BEGIN
			...
			IF
				(
					...
					WHERE
						ConfigType	= 'CREAgg'
					AND [ConfigParameter] = 'Enabled'
				) = 1
				BEGIN

					SELECT	DISTINCT
							MAX([PostingID]) OVER (PARTITION BY
													RecordTranSetID
													ORDER BY
													RNEMOID DESC
												) AS [PostingID],
							MAX(RnEMoID) OVER (PARTITION BY
												RecordTranSetID
												ORDER BY
												RNEMOID DESC
											)	AS [RnEMoID],
					...
					INTO
							#PostingId
					FROM
							[Posting].[RNEPostingExtract] AS PE
					WHERE
							RecordSettDate =
						(
							SELECT
					...
					AND RecordTranSetID IS NOT NULL
					AND RecordSourceMsg = 'MSG13';

					CREATE NONCLUSTERED INDEX nci_RecordTranSetID_RecordTranSetID
					ON #PostingId (RecordTranSetID, RecordTranSetID);
					...
					FROM
						[Posting].[RNEPostingExtract]			AS PE
					INNER JOIN
						[Posting].[vw_BeneficiaryPostingType]	AS BT
					ON
						BT.EntityId = PE.EntityId
					AND BT.PostingType = PE.RecordPostType
					INNER JOIN
						Config.PostingTypeSetting				AS PS
					ON
						PS.PostingType = BT.PostingType
					AND AggregatedPosting = 1
					INNER JOIN
						#PostingId								AS tmp
					ON
						PE.RecordTranSetID = tmp.RecordTranSetID
					AND PE.RecordSettDate = tmp.RecordSettDate
					WHERE
						BT.IsAggregated = 1
					OR	BT.IsAllItemsTriggered = 1;
					...
					FROM
						[Posting].[AggregatedItemsData] AS AG
					INNER JOIN
					...
					INNER JOIN
						(
							SELECT
								TransactionSetId
							FROM
								Posting.BeneficiaryPostingType AS BP
							WHERE
								BP.PostingID = @PostingID
							AND BP.IsAllItemsTriggered = 1
							AND DebitEntityState IN ( 260,
														261,
														262,
														263,
														280
					...
					FROM
						[Posting].[RNEPostingExtract]	AS PS
					INNER JOIN
						Staging.PostingEntity			AS PE
					ON
						PS.ItemIdentifier = PE.EntityIdentifier
					AND PS.PostingID = @PostingID;

					MERGE [Posting].[AggregatedItemsData] AS ER
					USING
						(
							SELECT
								[RnEMoID],
					...
					ON Ext.[ItemIdentifier] = ER.[ItemIdentifier]
					AND Ext.[RecordPostType] = ER.[RecordPostType]
					WHEN MATCHED AND (ER.[RecordSettDate] != Ext.[RecordSettDate])
					THEN UPDATE SET
					...
					THEN INSERT
						...
						VALUES
							(
								Ext.[RnEMoID], Ext.[PostingID], Ext.[HeaderSchema], Ext.[HeaderParticipant], Ext.[HeaderProcDate], Ext.[HeaderSequence], Ext.[HeaderVersion], Ext.[HeaderFileDate], Ext.[HeaderWeekDay], Ext.[HeaderType], Ext.[HeaderCurrency], Ext.[HeaderEnvironment], Ext.[EntityIdentifier], Ext.[PostingIdentifier], Ext.[ItemIdentifier], Ext.[EntityId], Ext.[SourceDateTime], Ext.[RecordSequence], Ext.[RecordPostType], Ext.[RecordSubType], Ext.[RecordSourceMsg], Ext.[RecordChannel], Ext.[RecordEntryType], Ext.[RecordPostSource], Ext.[RecordRespSource], Ext.[RecordPostDay], Ext.[RecordClearDate], Ext.[RecordSettDate], Ext.[RecordCaptDate], Ext.[RecordAmount], Ext.[RecordTranSetID], Ext.[RecordReason], Ext.[RecordOverride], Ext.[RecordNPASort], Ext.[RecordNPAAcc], Ext.[RecordSuppInfo], Ext.[RecordNumCheques], Ext.[RecordCollPart], Ext.[RecordCollLoc], Ext.[DebitTransID], Ext.[DebitSortCode], Ext.[DebitAccNum], Ext.[DebitSerNum], Ext.[DebitTranCode], Ext.[DebitSwitchSort], Ext.[DebitSwitchAcc], Ext.[DebitFullAmt], Ext.[CreditTransID], Ext.[CreditSortCode], Ext.[CreditAccNum], Ext.[CreditRef], Ext.[CreditTransCode], Ext.[CreditOrigAmt], Ext.[CreditOrigSort], Ext.[CreditOrigAcc], Ext.[CreditExcepCode], Ext.[CreditBeneficiary], Ext.[DetailDefSort], Ext.[DetailDefAcc], Ext.[DetailRepresent], Ext.[DetailFrontQual], Ext.[DetailRearQual], Ext.[DetailChannelRisk], Ext.[DetailFraudCode], Ext.[DetailFraudReason], Ext.[DetailFraudNameCheck], Ext.[DetailDuplicate], Ext.[DetailDupId], Ext.[DetailDupStatus], Ext.[DetailDupSeen], Ext.[DetailDupCollect], Ext.[DetailDupCapture], Ext.[DetailDupSource], Ext.[DetailStopped], Ext.[DetailStopDate], Ext.[DetailStopStatus], Ext.[DetailStopAmt], Ext.[DetailStopName], Ext.[DetailStopStart], Ext.[DetailStopEnd], Ext.[DetailRepSort], Ext.[DetailRepAcc], Ext.[DetailRepAmt], Ext.[DetailRepSer], Ext.[DetailException], Ext.[TrailerTransCount], Ext.[ItemPartitionSeq], Ext.[ExtractRecord]
							);
				END;
		END;
	END TRY
...
GRANT
	EXECUTE
ON [Posting].[usp_GetAggregatedItemsData]
TO
	[RNESVCAccess];
...
EXECUTE [sys].[sp_addextendedproperty]
	...
	@level1name = N'usp_GetAggregatedItemsData';


GO
EXECUTE [sys].[sp_addextendedproperty]
	@name = N'MS_Description',
	@value = N'Loads Aggregation items to Aggregation table for validation',
	@level0type = N'SCHEMA',
	@level0name = N'Posting',
	@level1type = N'PROCEDURE',
	@level1name = N'usp_GetAggregatedItemsData';
