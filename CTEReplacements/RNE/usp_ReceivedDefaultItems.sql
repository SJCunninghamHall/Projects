...
AS
    BEGIN TRY
        BEGIN
			...
			CREATE NONCLUSTERED INDEX nci_ParticipantID ON #ParticipantDetails(ParticipantID)
			...
			FROM
				(
					SELECT  
						...
						CR.AccountNumber 'Correct Account Number' ,
						CR.Reference AS 'Serial Number/Reference' ,
						Tst.Amount AS 'CR Value' ,
						0 AS 'DR Value'
					FROM    
						Report.dimDebitInformation DR
					INNER JOIN 
						[Report].[factDebitAmounts] Amt 
					ON 
						DR.DebitId = Amt.DebitId
					INNER JOIN 
						[Report].factCreditAmounts Tst 
					ON 
						Tst.TransactionSetKey = Amt.TransactionSetKey
					INNER JOIN 
						Report.dimCreditInformation CR 
					ON 
						CR.CreditId = Tst.CreditId
					INNER JOIN 
						Report.vw_DimTransactionSet DT 
					ON 
					...
					AND 
						CR.DefaultedSortcode = 1
					AND 
						DR.SettlementDate = @BusinessDate
					GROUP BY 
						CR.CreditId ,
						CR.Sortcode ,
						CR.AccountNumber ,
					...
					UNION ALL
						SELECT  
							...
							DR.AccountNumber 'Correct Account Number' ,
						...
						AND 
							DR.DefaultedSortcode = 1
						AND 
							DR.SettlementDate = @BusinessDate
				) ReceivedDefaultItems
			ORDER BY
...
GO
GRANT EXECUTE ON [WAR].[usp_ReceivedDefaultItems] TO [RnEWAR]
GO
