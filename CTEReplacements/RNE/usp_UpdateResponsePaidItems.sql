CREATE PROCEDURE [Posting].[usp_UpdateResponsePaidItems]
	(
		@ResponseID INT,
		@RneMoID	BIGINT
	)
...
AS
	BEGIN
		...
		BEGIN TRY
			...
			CREATE NONCLUSTERED INDEX nci_RTID_RPT_RNK
			ON #RNEPostingResp (ResponseTransID, ResponsePostType, RNK)
			INCLUDE (ResponseNPASortCode, ResponseNPAAccNum)
			WHERE RNK = 1;
			...
			FROM
				[Posting].[ExtractResponseDetails]	AS RP
			INNER JOIN
				#RNEPostingResp						AS PR
			ON
				PR.ResponseTransID = RP.ItemIdentifier
			AND PR.ResponsePostType = RP.RecordPostType
			WHERE
				[PR].[RNK] = 1;
...
GRANT
	EXECUTE
ON [Posting].[usp_UpdateResponsePaidItems]
TO
	[RNESVCAccess];
...
EXEC [sys].[sp_addextendedproperty]
	...
	@value = N'Update response items status',
	@level0type = N'SCHEMA',
	@level0name = N'Posting',
	@level1type = N'PROCEDURE',
	@level1name = N'usp_UpdateResponsePaidItems';
GO
