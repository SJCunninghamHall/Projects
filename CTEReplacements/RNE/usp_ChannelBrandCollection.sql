		...
		BEGIN TRY
			...
			DECLARE @ReportID INT =
						(
							...
							WHERE
								ReportName	= 'Report33ChannelBrandCollection'
						);
			...
			ON #UniqueParticipants (ParticipantID);
			...
			FROM
				(
					...
					AND [RejC].[RKD] = 1
					INNER JOIN
						Config.BrandMaster			AS BM
					ON
						BM.BrandID = EC.cf_BrandID
					INNER JOIN
						#UniqueParticipants			AS UP
					ON
...
GRANT
	EXECUTE
ON [RnEWareHouseReports].[usp_ChannelBrandCollection]
TO
	[RnEWAR];
GO
