	...
	BEGIN TRY
		...
		BEGIN
			...
			INTO
				#OutclearingIn
			FROM
				[Report].[dimCreditEntityStateHistory]	AS I
			INNER JOIN
				#EligibleItems							AS EI
			ON
			...
			CREATE NONCLUSTERED INDEX nci_CreditId_EntRank
			ON #OutclearingIn (CreditId, EntRank);
			...
			CREATE NONCLUSTERED INDEX nci_CreditId_EntRank
			ON #MAX_Revision (CreditId, EntRank);
			...
			USING
				...
				(CreditId, Channel, IsOutclearingIn, IsHoldover, IsWithdrawn, IsOutclearingOut, OriginalAmount, Amount, FinalEntityState, IsItemSubmitToSwitch)
			ON EFC.ItemTransID = NFC.CreditId
			...
			THEN INSERT
				...
				VALUES
					(
						NFC.CreditId, @BusinessDate, NFC.Channel, NFC.IsOutclearingIn, NFC.IsHoldover, NFC.IsHoldover, NFC.IsWithdrawn, NFC.IsOutclearingOut, ISNULL(NFC.OriginalAmount, 0), ISNULL(NFC.Amount, 0), 'Cr', NFC.FinalEntityState, NFC.IsItemSubmitToSwitch
					);
...
GRANT
	EXECUTE
ON [DataImport].[usp_UpdateCollectingReconciliationCredits]
TO
