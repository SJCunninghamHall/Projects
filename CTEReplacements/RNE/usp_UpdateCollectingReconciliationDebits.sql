        ...
        BEGIN				
			...
			(
				SELECT 
					DebitId
					,ICSAmount
					,ROW_NUMBER() OVER ( PARTITION BY DebitId ORDER BY Revision DESC) Rnk
					,MSG01
					,0 As MSG02
					,0 As MSG03
				FROM 
				...
				UNION ALL
					...
					FROM 
						[Report].dimDebitEntityStateHistory
					WHERE 
						DateKey=@BusinessDateInt AND MSG02 > 0
					UNION ALL
						SELECT 
							DebitId
							,ICSAmount
			...
			SELECT 
				...
				I.DateKey,
				DebitEntryDate,
				ROW_NUMBER() OVER (PARTITION BY I.DebitId ORDER BY EntityId DESC) EntRank
			INTO 
				#OutclearingIn
			FROM 
				[Report].[dimDebitEntityStateHistory] I
			INNER JOIN 
				#EligibleItems EI 
			ON 
				EI.DebitId = I.DebitId
			WHERE 
				I.MSG01 IN (20,21,23,25) 
			AND 
				EntityType = 'I'
...
GRANT EXECUTE ON [DataImport].[usp_UpdateCollectingReconciliationDebits] TO [RnEReportDwDataImporter];
