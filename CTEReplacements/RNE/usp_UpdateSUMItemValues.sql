...
AS
	BEGIN  
		...
		BEGIN TRY  
			...
			DECLARE @Businessdate DATE,@PrevBusinessDate DATE,@NextBusinessDate DATE
			...
			EXEC [Base].[usp_LogEvent] 1,
				'[Posting].[usp_UpdateSUMItemValues]', 'Enter';    
				...
				EXISTS 
					(   
						SELECT 
							1 
						FROM 
							[Config].[SumMsgEntityState] cfn   
						WHERE 
							(
								Db.MessageType = cfn.MessageType
							AND 
								Db.EntityState = cfn.EntityState
							AND 
								PaidInd = 'N'
							)
					)
			...
			CREATE NONCLUSTERED INDEX nci_DebitId ON #ExcludeDebits(DebitId)
			...
			CREATE NONCLUSTERED INDEX nci_DebitId ON #IncludeDebits(DebitId)
			...
			SELECT 
				TransactionSetId,
				Amount,
				ROW_NUMBER() OVER(PARTITION BY DebitId ORDER BY Db.MessageType DESC) LatestDebit 
			INTO
			   #getLatest
			FROM 
				[Staging].[DebitMsgES] Db 
			...
			ON 
				...
				AND 
					(
						Db.EntityState NOT IN (210,211) 
					AND 
						Db.SettlementDate=@Businessdate
					)
				)
			WHERE 
				NoOfDebit > 1    
			...
			CREATE NONCLUSTERED INDEX nci_TransactionSetId ON #AGGDBAmt(TransactionSetId)
			...
			AND 
				Cr.MessageType NOT IN ('MSG06')
			AND 
				Db.NPASortCode IS NOT NULL AND Cr.NPASortCode IS NULL;
			...
			FROM 
				Outclearing.SumDayOneExtracts TB 
			INNER JOIN 
				( 
					SELECT 
						DP.TransactionSetIdWithVersion
					FROM 
						Staging.DerivedPostingType DP
					WHERE 
						DP.CreditEntityState IN (165)
					AND 
						DP.ProcessingDate=@BusinessDate
					GROUP BY 
						DP.TransactionSetIdWithVersion
				) CTR 
			ON 
			...
			EXEC [Base].[usp_LogEvent] 1,
			'[Posting].[usp_UpdateSUMItemValues]', 'Exit';    
...
GRANT EXECUTE ON [Posting].[usp_UpdateSUMItemValues] TO [RNESVCAccess];
	
GO
EXEC sys.sp_addextendedproperty @name = N'MS_Description',
    @value = N'Update SUm Debit and Credit Entries for final Extract', @level0type = N'SCHEMA',
    @level0name = N'Posting', @level1type = N'PROCEDURE',
    @level1name = N'usp_UpdateSUMItemValues';
GO

EXEC sys.sp_addextendedproperty @name = N'Version', @value = N'$(Version)',
    @level0type = N'SCHEMA', @level0name = N'Posting',
