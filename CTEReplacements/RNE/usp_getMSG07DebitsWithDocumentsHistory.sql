...
BEGIN
	...
	FROM
		(
			SELECT 
				...
				,E.EntityIdentifier
				,E.EntityState
				,ROW_NUMBER() OVER (PARTITION BY E.EntityIdentifier ORDER BY E.Revision DESC) AS RKD
			FROM 
				Base.Entity E
			JOIN 
				Base.Core C 
			...
			AND 
				E.EntityType = 'D' 
			AND 
				C.CoreId BETWEEN @BusinessDateRangeStart AND @BusinessDateRangeEnd
		) A

	CREATE NONCLUSTERED INDEX nci_EntityIdentifier ON #MSG08EntityStates (EntityIdentifier)
	...
	FROM
		(
			...
			FROM 
				Base.Entity E
			JOIN 
				Base.Core C ON C.CoreId=E.CoreId
			WHERE 
	...
	CREATE NONCLUSTERED INDEX nci_EntityIdentifier ON #MSG09EntityStates (EntityIdentifier)
	...
	CREATE NONCLUSTERED INDEX nci_EntityIdentifier ON #RepairEntityStates (EntityIdentifier)
	...
	(
		...
		FROM
			Base.Debit DB
		WHERE
			ItemId BETWEEN @BusinessDateRangeStart AND @BusinessDateRangeEnd
		AND 
	...
	SELECT 
		...
		,MSG02EntityState AS MSG02
		,MSG03EntityState AS MSG03
		,MSG03ErrorCode
	FROM 
		#FinalDocsCTE F
...
Grant Execute ON [RNEReport].[usp_getMSG07DebitsWithDocumentsHistory] to [RNEReportAccess]
