...
AS
	BEGIN
		...
		SELECT
			...
			CTEInitialEntities.ChargedParticipantId				AS ChargingParticipant,
			...
			MAX(CTEInitialEntities.RejEntityState) OVER (PARTITION BY
															CTEInitialEntities.TransId
															ORDER BY
															CTEInitialEntities.TransId
														)		AS RejEntityState,
			MAX(CTEInitialEntities.DocumentEntityState) OVER (PARTITION BY
																CTEInitialEntities.TransId
																ORDER BY
																CTEInitialEntities.TransId
															) AS DocumentEntityState,
			MAX(CTEInitialEntities.DefaultPayDecision) OVER (PARTITION BY
																CTEInitialEntities.TransId
																ORDER BY
																CTEInitialEntities.TransId
															)	AS DefaultPayDecisionInd
		INTO
			#FinalEntities
		FROM
			[RNE].[StagingCTEInitialEntities] AS CTEInitialEntities;
		...
		CREATE NONCLUSTERED INDEX nci_iefe_FinalRKD
		ON #FinalEntities (isEligibleForExtract, FinalRKD)
		WHERE isEligibleForExtract = 1
		AND FinalRKD = 1;
		...
		SELECT
			...
			CASE
				WHEN [FinalEntities].[RejEntityState] <> 0
				THEN COALESCE(	CAST(FinalEntities.Day2ResponseWindowEndDateTime AS DATE),	--PICK THIS up if exists
				...
				THEN @BusinessDate
				ELSE COALESCE(	CAST(FinalEntities.Day2ResponseWindowEndDateTime AS DATE),	--PICK THIS up if exists
								CAST(LEFT(FinalEntities.MSG13ItemId, 8) AS DATE),			--the date on which MSG13 is receive is SettlementDate	
			...
			SUM(ResAmount) OVER (PARTITION BY
									TranSetID
								)													AS DBSumResAmount,
			MAX([FinalEntities].[CashInd]) OVER (PARTITION BY
													FinalEntities.TranSetID
													ORDER BY
													FinalEntities.TranSetID
												)									AS DebitCashInd
		FROM
			#FinalEntities AS FinalEntities
		WHERE
			FinalEntities.isEligibleForExtract = 1
		AND FinalEntities.FinalRKD = 1;
...
GRANT
	EXECUTE
ON [RNE].[usp_CTE_ReturnAllDebits]
TO
	[RNESVCAccess];
...
EXEC [sys].[sp_addextendedproperty]
	...
	@level1name = N'usp_CTE_ReturnAllDebits';
GO

EXEC [sys].[sp_addextendedproperty]
	@name = N'MS_Description',
	@value = N'This stored procedure returns final set of Debits for CTE.',
	@level0type = N'SCHEMA',
	@level0name = N'RNE',
	@level1type = N'PROCEDURE',
	@level1name = N'usp_CTE_ReturnAllDebits';
