...
SET NOCOUNT ON;
	...
	BEGIN TRY
		...
		DECLARE @NoOfTxnsPerFile INT =
					(
						SELECT
							CAST(ConfigValue AS INT)
						FROM
							Config.ApplicationConfig AS W
						WHERE
							ConfigType	= 'AGENCY'
		...
		DECLARE @Namespace VARCHAR(300) =
					(
						...
						WHERE
							[ConfigType] = 'AGENCY'
						AND [ConfigParameter] = @MessageType
					);
		...
		WHERE
				AgencyId = @AgencyID
		AND IntMessageType = @MessageType
		AND ISNULL(IsExtracted, 0) = 0;
		...
		WHERE
			AI.AgencyId = @AgencyID
		AND AI.IntMessageType = @MessageType
		GROUP BY
			AI.TxSetId,
			Credit_CdtItmId;
		...
		SELECT
			...
			(
				SELECT
					AI	.[TxSetId],
					RIGHT('00' + CONVERT(VARCHAR, [TxSetVrsn]), 2) AS TxSetVrsn,
					[ColltngPtcptId],
					[CaptrdDtTm],
			...
			(
				...
				FROM
					Staging.AgencyISOItem	AS CR_AI
				INNER JOIN
					#Txn_Cr_CTE				AS CR
				ON
			...
			(
				...
				FROM
					Staging.AgencyISOItem	AS Db_AI
				INNER JOIN
					#Txn_Db_CTE				AS Db
				ON
				...
				FOR XML PATH('TxSetSubmissn'), TYPE
			)
				FOR XML PATH('TxSetSubmissn'), TYPE
			);
...
GRANT
	EXECUTE
ON [Staging].[usp_GetAgencyISOExtractXML]
TO
	[RNESVCAccess];
