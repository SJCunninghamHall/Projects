	...
	BEGIN TRY
		...
		BEGIN	
			...
			INTO
				#MSG08AckRcvd
			FROM 
				Report.dimReportEntityState ES 
			INNER JOIN 
				Report.dimReportMaster RM
			ON 
			...
			CREATE NONCLUSTERED INDEX nci_EntityState ON #MSG08AckRcvd(EntityState)
			...
			WHERE	
				ReportName = 'Inclearing Submission Repair Report'
			AND 
				ColumnName = 'NegAckRcvd' 
			AND 
				MessageType = 'MSG08'

			CREATE NONCLUSTERED INDEX nci_EntityState ON #MSG08NegAckRcvd(EntityState)
			...
			CREATE NONCLUSTERED INDEX nci_EntityState ON #MSG09Rejected(EntityState)
			...
			CREATE NONCLUSTERED INDEX nci_EntityState ON #MSG09Repaired(EntityState)
			...
			SELECT		
				...
				,CASE 
					WHEN 
						DC.MSG08 IN (SELECT EntityState FROM #MSG08NegAckRcvd) 
						OR 
						DC.MSG09 IN (SELECT EntityState FROM #MSG09Rejected) 
					THEN 1 
					ELSE 0 
				END AS TotalRejected  
				,CASE 
					WHEN DC.RepairES IN (SELECT EntityState FROM #MSG09Repaired) 
					THEN 1 
					ELSE 0 
				...
				,CASE 
					WHEN 
						...
						DC.MSG09 IN (SELECT EntityState FROM #MSG09Rejected) 
					THEN 
						ISNULL(DC.MSG09ErrorCode,'')
					WHEN 
						(DC.MSG08 IN (SELECT EntityState FROM #MSG08AckRcvd) ) 
						AND 
						DC.MSG09 IN (SELECT EntityState FROM #MSG09Rejected) 
					THEN 
						ISNULL(DC.MSG09ErrorCode,'')
			...
			AND
				...
				OR 
					(
						DC.MSG09 IN (SELECT EntityState FROM #MSG09Rejected) 
					OR 
						DC.RepairES IN (SELECT EntityState FROM #MSG09Repaired)	
					)
				OR
					(
						DC.MSG08 IN (SELECT EntityState FROM #MSG08NegAckRcvd) 
			...
			FROM 
				MSG07DocNakItems MD
			INNER JOIN 
				[Report].[dimAgencySortCodeList] AG 
			ON 
...
Grant Execute ON [DataImport].[usp_InsertInclearingSubmissionHistory] to [RnEReportDwDataImporter]
