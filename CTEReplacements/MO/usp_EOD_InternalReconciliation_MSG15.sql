    ...
    BEGIN
        ...
        BEGIN TRY
            ...
            SELECT
			 ...
			 SELECT
                @MessageAckState
                = [$(MoConfigDatabase)].[Config].[sfn_ReadScalarConfigValue]('MessageAckState','02SM01');
			...
			AND
				src.SourceState = @MessageAckState
			AND 
				BusinessDate = @BusinessDate

			CREATE CLUSTERED INDEX ci_SourceID ON #AckCTE(SourceID)
			...
			IF (@SubmissionRprtNotRcvdCount = 0)
			  BEGIN
			   ...
			   WHERE
                  MessageType = '15SM01'
				  AND EXISTS (SELECT 1 FROM #EODStates WHERE DocumentState = SourceState)
			      AND SourceDirtyFlag != @SourceDirtyFlag
			      AND BusinessDate = @BusinessDate
			 END
			        
            IF (@SubmissionRprtNotRcvdCount = 0 AND @NotArchiveCount = 0)
                BEGIN
                ...
                BEGIN
                    ...
                    SET @Message
                        = '[MOException] EOD- MO Internal ' + @DownstreamComponent + ' Reconciliation process for '
                          + @RoleFlag + ' ' + @Window + ' failed.' + CAST(@SubmissionRprtNotRcvdCount AS VARCHAR(10))
                          + ' Document(s) have not received submission report from SWITCH.';
