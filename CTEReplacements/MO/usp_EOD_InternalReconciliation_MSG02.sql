		...
		BEGIN TRY
			...
			ELSE IF @NotAcknowledgeCount > 0
					BEGIN
						...
						SET @Severity = 3;
						SET @Message = '[MOException] EOD- MO Internal ' + @DownstreamComponent + ' Reconciliation process for ' + @RoleFlag + ' ' + @Window + ' failed.' + CAST(@NotAcknowledgeCount AS VARCHAR(10)) + ' Document(s) have not received acknowledgement from SWITCH.';
