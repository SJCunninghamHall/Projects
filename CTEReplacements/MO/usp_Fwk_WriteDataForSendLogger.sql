...
DECLARE
	...
	BEGIN
		...
		BEGIN TRY
			...
			AND		[Base].ItemState.PayerItemState IN
						(
							SELECT
								StringColumnB
							FROM
			...
			AND		[Base].ItemState.CollectorItemState IN
						(
							SELECT
								StringColumnB
							FROM
			...
			AND		[Base].ItemState.BeneficiaryItemState IN
						(
							SELECT
								StringColumnB
							FROM
			...
			AND		transactionSetState.CollectorTsetState IN
						(
							SELECT
								StringColumnB
							FROM
			...
			AND		transactionSetState.BeneficiaryTsetState IN
						(
							SELECT
								StringColumnB
							FROM
			...
			TsetID	= sta.TsetID,
				...
				State = CASE
							WHEN @participantType = 'Payer'
							AND copyItem.[State] IS NULL
							THEN sta.PayerItemState
							WHEN @participantType = 'Collector'
							AND copyItem.[State] IS NULL
							THEN sta.CollectorItemState
							WHEN @participantType = 'Beneficiary'
							AND copyItem.[State] IS NULL
			...
			State	= CASE
							WHEN @participantType = 'Payer'
							AND sta.PayerItemState IS NOT NULL
							THEN sta.PayerItemState
							WHEN @participantType = 'Collector'
							AND sta.CollectorItemState IS NOT NULL
							THEN sta.CollectorItemState
							WHEN @participantType = 'Beneficiary'
							AND sta.BeneficiaryItemState IS NOT NULL
							THEN sta.BeneficiaryItemState
							WHEN @participantType = 'ALL'
							AND copyItem.State IS NOT NULL
							THEN copyItem.State
							WHEN @participantType = 'Holdover'
							AND copyItem.State IS NOT NULL
							THEN copyItem.State
			...
			IF (
				@ExternalMsgType = 'PSTNG'
			AND		@DestinationSubSystem IN (	'DEW',
												'IA'
											)
			)
				BEGIN
					...
					IF (@messageType = 'PTMA01')
						BEGIN
							...
							AND pes.ClearingState = cpy.[State]
							AND pes.PostingState = @PayloadCreatedState
							AND pes.TsetID = cpy.TsetID;
						END;
					ELSE IF (@messageType = 'POTMA1')
							BEGIN
								...
								FROM
									Base.PostingEntryState	AS pes
								INNER JOIN
									@CopyTransactionSet		AS tset
								ON
									pes.TsetID = tset.TsetID
								AND pes.ClearingState = tset.[State]
								AND pes.PostingState = @PayloadCreatedState;
							END;
					...
					ELSE IF (
								@messageType = 'PEMA01'
						OR		@messageType = 'POEMA1'
						OR		@messageType = 'PEMA02'
						OR		@messageType = 'PSMA01'
							)
							BEGIN
								DECLARE @ParentSourceIds	VARCHAR(100);
								SELECT
									@ParentSourceIds	= CAST(il.StringColumnB AS VARCHAR(100))
								FROM
									@IdsList AS il
								WHERE
					...
					ELSE IF (
								@messageType = 'PRMA01'
						OR		@messageType = 'PORMA1'
							)
							BEGIN
								DECLARE @ParentSourceId VARCHAR(100);
								SELECT
									@ParentSourceId = CAST(il.StringColumnB AS VARCHAR(100))
								FROM
									@IdsList AS il
								WHERE
									il.StringColumnA = 'Document';
									...
									(
										SELECT
											...
											ROW_NUMBER() OVER (PARTITION BY
																pe.PostingEntryID
																ORDER BY
																pes.PostingEntryStateSKID DESC
															) AS RowNumber
										FROM
											[Base].PostingEntry			AS pe
										INNER JOIN
											[Base].PostingEntryState	AS pes
										ON
										...
										AND RIGHT(pe.PostingEntryID, 3) = 'DEB'
									) AS tmp
								WHERE
									[tmp].[RowNumber] = 1;
							END;
			...
			State	= CASE
						...
						END
			FROM
				@CopyTransactionSet			AS ts
			INNER JOIN
				[Base].TransactionSetState	AS tss
			ON
