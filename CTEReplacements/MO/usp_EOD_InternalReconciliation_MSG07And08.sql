		...
		BEGIN TRY
				...
				BEGIN
					IF	(@Window = 'Day 2')
						BEGIN
							...
							AND itmState.PayerItemBatchedFlag != @DebitBatchFlag_1;
							...
							IF @ItemNotSentToSwitch = 0
								BEGIN
									...
									AND PayerItemDirtyFlag != @DebitDirtyFlag_1;
										...
										BEGIN
											...
											IF @DocNotAcknowledged = 0
												BEGIN
													...
													OR		SourceDirtyFlag != @SourceDirtyFlag
														)
													AND BusinessDate = @BusinessDate;
													...
													IF @DocumentsNotArchived = 0
														BEGIN
															...
															AND PayerItemDirtyFlag != @DebitDirtyFlag_2;
														END;
												END;
										END;
								END;
			...
			AND		@ReconItemCount = 0
				...
				BEGIN
					...
					SET @ErrorCode = '';
					SET @Message = 'EOD- MO Internal ' + @DownstreamComponent + ' Reconciliation process for ' + @RoleFlag + ' ' + @Window + ' completed successfully.';
					SET @Severity = 1;
					SET @SysOut = '';
				END;
			ELSE IF @ItemNotSentToSwitch > 0
					BEGIN
						...
						SET @Severity = 3;
						SET @Message = '[MOException] EOD- MO Internal ' + @DownstreamComponent + ' Reconciliation process for ' + @RoleFlag + ' ' + @Window + ' failed.' + CAST(@ItemNotSentToSwitch AS VARCHAR(10)) + ' Item(s) have not been sent to ' + @DownstreamComponent + '.';
			...
			ELSE IF @ItemNotArchived > 0
					BEGIN
						...
						SET @Result = [$(MoConfigDatabase)].[config].sfn_ReadScalarConfigValue('EOD', 'FailureCode');
						SET @Severity = 3;
						SET @Message = '[MOException] EOD- MO Internal ' + @DownstreamComponent + ' Reconciliation process for ' + @RoleFlag + ' ' + @Window + ' failed.' + CAST(@ItemNotArchived AS VARCHAR(10)) + ' Item(s) sent to ' + @DownstreamComponent + ' have not been ARCHIVED.';
						SET @SysOut = @ErrorCode + ':' + @Message;
					END;
			ELSE IF @DocNotAcknowledged > 0
					BEGIN
						...
						SET @Result = [$(MoConfigDatabase)].[config].sfn_ReadScalarConfigValue('EOD', 'FailureCode');
						SET @Severity = 3;
						SET @Message = '[MOException] EOD- MO Internal ' + @DownstreamComponent + ' Reconciliation process for ' + @RoleFlag + ' ' + @Window + ' failed.' + CAST(@DocNotAcknowledged AS VARCHAR(10)) + ' Documents(s) have not received acknowledgement from SWITCH.';
						SET @SysOut = @ErrorCode + ':' + @Message;
					END;
			ELSE IF @DocumentsNotArchived > 0
					BEGIN
						...
						SET @Result = [$(MoConfigDatabase)].[config].sfn_ReadScalarConfigValue('EOD', 'FailureCode');
						SET @Severity = 3;
						SET @Message = '[MOException] EOD- MO Internal ' + @DownstreamComponent + ' Reconciliation process for ' + @RoleFlag + ' ' + @Window + ' failed.' + CAST(@DocumentsNotArchived AS VARCHAR(10)) + ' Documents(s) have not been sent to ARCHIVE.';
						SET @SysOut = @ErrorCode + ':' + @Message;
					END;
			ELSE IF @ReconItemCount > 0
					BEGIN
						...
						SET @Result = [$(MoConfigDatabase)].[config].sfn_ReadScalarConfigValue('EOD', 'FailureCode');
						SET @Severity = 3;
						SET @Message = '[MOException] EOD- MO Internal ' + @DownstreamComponent + ' Reconciliation process for ' + @RoleFlag + ' ' + @Window + ' failed.' + CAST(@ReconItemCount AS VARCHAR(10)) + ' Items(s) have not been sent to ARCHIVE.';
						SET @SysOut = @ErrorCode + ':' + @Message;
					END;
