...
AS
	BEGIN
		...
		BEGIN TRY
			...
			DECLARE @PostingRetriggerState VARCHAR(1000) = ',' + [$(MoConfigDatabase)].[config].[sfn_ReadScalarConfigValue]('EOD', 'PostingRetriggerState') + ','; --948
			DECLARE @PEMA01States VARCHAR(50) = ',' + [$(MoConfigDatabase)].[config].sfn_ReadScalarConfigEAVValue('EOD', 'PEMA01States') + ','; --950,975,949,947
			DECLARE @NonExtractState VARCHAR(1000) = ',' + [$(MoConfigDatabase)].[config].[sfn_ReadScalarConfigValue]('EOD', 'NonExtractState') + ','; -- 945
			DECLARE @ExtractMessageTypes VARCHAR(1000) = ',' + [$(MoConfigDatabase)].[config].[sfn_ReadScalarConfigValue]('EOD', 'IntMsgTypeExtractArchive') + ','; --PEMA01			
			DECLARE @PayerBenTriggerableStates VARCHAR(1000) = ',' + [$(MoConfigDatabase)].[config].[sfn_ReadScalarConfigValue]('EOD', 'PayerBenTriggerableStates') + ',';
			DECLARE @06Dm01ClearingState VARCHAR(1000) = ',' + [$(MoConfigDatabase)].[config].[sfn_ReadScalarConfigValue]('EOD', '06Dm01ClearingState') + ','; --560,561
			DECLARE @Day1ClearingState VARCHAR(1000) = ',' + [$(MoConfigDatabase)].[config].[sfn_ReadScalarConfigValue]('EOD', '06Day1PostableClearingState') + ','; --520
			DECLARE @06Km01PostingState VARCHAR(1000)	= ',' + [$(MoConfigDatabase)].[config].[sfn_ReadScalarConfigValue]('EOD', '06Km01PostingState') + ','; --970
			DECLARE @SentToBankSuccessPostingStates VARCHAR(1000)	= ',' + [$(MoConfigDatabase)].[config].[sfn_ReadScalarConfigValue]('EOD', 'SentToBankSuccess') + ','; --955
			DECLARE @SentToBankCDFailurePostingStates VARCHAR(1000) = ',' + [$(MoConfigDatabase)].[config].[sfn_ReadScalarConfigValue]('EOD', 'SentToBankConnectDirectFailure') + ','; --956
			DECLARE @SentToBankOtherFailurePostingStates VARCHAR(1000) = ',' + [$(MoConfigDatabase)].[config].[sfn_ReadScalarConfigValue]('EOD', 'SentToBankOtherFailure') + ','; --957,958
			DECLARE @ExtractSuccessStates VARCHAR(1000) = ',' + [$(MoConfigDatabase)].[config].[sfn_ReadScalarConfigValue]('EOD', 'PendingResponseStates') + ','; --950
			DECLARE @PostingRollOverES INT = [$(MoConfigDatabase)].[config].[sfn_ReadScalarConfigValue]('PostingRollOver', 'PostingRollOverEntityState'); --901
			...
			FROM
				STRING_SPLIT(@PostingRetriggerState, ',') AS PostingRetriggerState
			CROSS APPLY
				(
					SELECT
						value
			...
			FROM
				STRING_SPLIT(@06Km01PostingState, ',')	AS Km01PostingState
			CROSS APPLY
				(
					SELECT
						value
			...
			SELECT
				...
				[Dm01ClearingState].[value]
			FROM
				STRING_SPLIT(@06Dm01ClearingState, ',') AS Dm01ClearingState
			CROSS APPLY
				(
					SELECT
						value
			...
			FROM
				STRING_SPLIT(@SentToBankOtherFailurePostingStates, ',') AS SentToBankOtherFailurePostingStates
			WHERE
				[SentToBankOtherFailurePostingStates].[value] != '';
			...
			FROM
				STRING_SPLIT(@PayerBenTriggerableStates, ',') AS PayerBenTriggerableStates
			CROSS APPLY
				(
					SELECT
						value
			...
			CROSS APPLY
				(
					...
					WHERE
						value != ''
				)												AS SentToBankCDFailurePostingStates
			WHERE
				[PayerBenTriggerableStates].[value] != '';
			...
			CROSS APPLY
				(
					...
					WHERE
						value != ''
				)												AS SentToBankSuccessPostingStates
			WHERE
				[PayerBenTriggerableStates].[value] != '';

			EXEC [Base].[usp_UpdatePostingEntityState]
				@ActivityID;
			...
			AND EXISTS
				(
					...
					OR		(PostingState IS NULL)
						)
					AND ClearingState = CollectorState	--exclude 06dm01 scenario
				);
			...
			WHERE
				...
				(
					...
					AND PostingState = ResponseState	--970
				)
			AND
			(
					Window1BusinessDate = @BusinessDate
			OR		Window2BusinessDate = @BusinessDate
				);

			CREATE CLUSTERED INDEX ci_IID_TID
			ON #CTE (ItemID, TsetID);

			SELECT
				@itemsNotTriggeredFor06DM01 = COUNT(1)
			FROM
				[Base].PostingEntryState	AS ps
			INNER JOIN
				#CTE						AS cte
			ON
				cte.ItemID = ps.ItemID
			AND cte.TsetID = ps.TsetID
			WHERE
				(
					Window1BusinessDate = @BusinessDate
			OR		Window2BusinessDate = @BusinessDate
			...
			AND EXISTS
				(
					...
					OR		PostingState = RetriggerState
						)
				);	--948
			...
			WHERE
				Window2BusinessDate = @BusinessDate
			AND ClearingState = @PostingRollOverES
			AND EXISTS
				(
					SELECT
						1
				...
				BEGIN
					...
					AND EXISTS
						(
							...
							AND ClearingState = CollectorState
						);
						...
						BEGIN
							...
							AND EXISTS
								(
									...
									AND PostingState = ExtractedState --950,949,975  
									AND ClearingState = CollectorState
								);
								...
								BEGIN
									...
									AND EXISTS
										(
											...
											WHERE
												InternalMessageType = MessageType --PEMA01
											AND PostingState = ExtractedState --950
											AND ClearingState = CollectorState
										);
										...
										BEGIN
											...
											IF @itemsNotSentToBank = 0
												BEGIN
													...
													WHERE
														Window2BusinessDate = @BusinessDate
													AND InternalMessageType != 'PSMA01'
													AND EXISTS
														(
															SELECT
																1
													...
													ELSE
														BEGIN
															SET @decision = 0;
															SET @message = SUBSTRING(@message + CAST(@itemsSentToBankNotArchv AS VARCHAR(20)) + ' item(s) was/were sent to bank for payer/ben were not archived.', 1, 500);
														END;
												END;
											ELSE
												BEGIN
													SET @decision = 0;
													SET @message = SUBSTRING(@message + CAST(@itemsNotSentToBank AS VARCHAR(20)) + ' item(s) was/were extracted but not sent to bank for payer/ben.', 1, 500);
												END;
										END;
									ELSE
										BEGIN
											SET @decision = 0;
											SET @message = SUBSTRING(@message + CAST(@itemNotReceivedPSRM01 AS VARCHAR(20)) + ' item(s) was/were extracted but PSRM01 not received for payer/ben.', 1, 500);
										END;
								END;
							ELSE
								BEGIN
									SET @decision = 0;
									SET @message = SUBSTRING(@message + CAST(@itemsNotArchived AS VARCHAR(20)) + ' item(s) was/were extracted but not archived.', 1, 500);
					...
					ELSE
						BEGIN
							SET @decision = 0;
							SET @message = SUBSTRING(@message + CAST(@itemsNotExtracted AS VARCHAR(20)) + ' item(s) have/has not been extracted.', 1, 500);
			...
			ELSE
				BEGIN
					SET @decision = 0;
					SET @message = SUBSTRING(@message + CAST((@itemsNotTriggered) AS VARCHAR(20)) + ' item(s) have/has not been triggered for postings.', 1, 500);
				END;

			IF (@decision = 1)
				BEGIN
					SET @Result = [$(MoConfigDatabase)].[config].sfn_ReadScalarConfigValue('EOD', 'SuccessCode');
					SET @message = SUBSTRING('EOD - MO Posting extract reconciliation process for payer/beneficiary completed successfully.', 1, 500);
					SET @severity = 1;
					SET @Sysout = '';
				END;
			ELSE
				BEGIN
					...
					SET @severity = 3;
					SET @message = SUBSTRING('[MOException] EOD - MO Posting Extract reconciliation process for payer/beneficiary failed.' + @message, 1, 500);
