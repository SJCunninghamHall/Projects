	...
	BEGIN
		...
		BEGIN TRY
			...
			INTO
				#CTE
			FROM 
				Base.EISCDParticipantMaster EPM 
			INNER JOIN 
				[RefOut].[udf_GetAllBankParticipantID]() GBC 
			ON 
				EPM.ParticipantId = GBC.ParticipantId

			CREATE CLUSTERED INDEX nci_ParticipantId ON #CTE(ParticipantId, BankCode)
			...
			INNER JOIN 
				Manual.AgencySortCode FOR SYSTEM_TIME AS OF @Date AGSC 
			ON 
			...
			OR 
				...
				AND 
					(
						(
							AM.AgencyType IN (SELECT value FROM STRING_SPLIT (@AgencyType,','))
						) 
						OR 
							AM.AgencyType IS NULL
					)
				)
			...
			INNER JOIN 
				(
					SELECT 
						CT.Bankcode 
					FROM
						Base.EISCDParticipantMaster PM 
					INNER JOIN 
						#CTE CT 
					ON 
		...
		--- RESULT SET 3 - It is a resultset of SortCode for Group Bank exists in AgencySortCode table
			...
			ON 
				AM.ParentParticipantId = GBC.ParticipantId
			INNER JOIN 
				Manual.AccessibleAgency FOR SYSTEM_TIME AS OF @Date AA 
			ON 
...
END
GO
EXEC sys.sp_addextendedproperty @name=N'Component', @value=N'iPSL.ReferenceDataDB' , @level0type=N'SCHEMA',@level0name=N'RefOut', @level1type=N'PROCEDURE',@level1name=N'usp_GetAgencySortCodeData'
GO
EXEC sys.sp_addextendedproperty @name=N'MS_Description', @value=N'This Stored Procedure will fetch the records from AgencySortCode table' , @level0type=N'SCHEMA',@level0name=N'RefOut', @level1type=N'PROCEDURE',@level1name=N'usp_GetAgencySortCodeData'
GO
EXEC sys.sp_addextendedproperty @name=N'Version', @value=N'$(Version)' , @level0type=N'SCHEMA',@level0name=N'RefOut', @level1type=N'PROCEDURE',@level1name=N'usp_GetAgencySortCodeData'
GO
GRANT EXECUTE
    ON OBJECT::[RefOut].[usp_GetAgencySortCodeData] TO [Reference_Lloyds_Access]
GO
GRANT EXECUTE
    ON OBJECT::[RefOut].[usp_GetAgencySortCodeData] TO [Reference_HSBC_Access];