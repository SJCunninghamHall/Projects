...
BEGIN TRY
	...
	INSERT INTO 
		...
		FROM 
			@TransactionSet sourceInput

	;WITH CTE AS (
		SELECT 
			ROW_NUMBER() OVER (PARTITION BY TsetID, NodeType ORDER BY (SELECT 0)) RowNumber 
		FROM 
			@CopyTransactionSet
	)
	DELETE FROM 
	...
	FROM
		@PostingEntry postingInput


	;WITH CTE AS 
	(
		SELECT 
			ROW_NUMBER() OVER (PARTITION BY TsetID, ItemID, PostingEntryID ORDER BY (SELECT 0)) RowNumber 
		FROM 
			@CopyPostingEntry
	)
	...
	SELECT 
		...
		,RowId 
	FROM 
		@postingEntryState postingStateInput
	
	-- Update missing fields for the records. Join will be at TsetID
	IF EXISTS (SELECT 1 FROM @CopyPostingEntryState WHERE OperationType = 0)
	BEGIN
		UPDATE 
			copypes
		SET
		...
		AND
			copypes.PreviousPostingState = pes.PostingState
		AND
			pes.SourceID IN (SELECT SourceID FROM @Source WHERE OperationType = 1)
	...
	AND
		(
		 ...
		 OR
			copypes.PostingEntryId= pes.PostingEntryId
		)
	...
	BEGIN TRANSACTION
		...
		EXEC [Base].[usp_Fwk_UpsertSource] @Source, @ActivityId, @sourceSkId OUTPUT		
					
		-- Update sourceskid into @CopyTransactionSet
		UPDATE @CopyTransactionSet
				SET SourceSKID = @sourceSkId
		...
		EXEC [Base].[usp_Fwk_UpsertPostingEntry] @CopyPostingEntry,@ActivityId
...
GRANT EXECUTE ON OBJECT:: [Base].[usp_Fwk_WriteDataForPostings] TO [WebRole]
AS [dbo]
GO