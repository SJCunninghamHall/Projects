...
DECLARE
	...
	BEGIN
		...
		BEGIN TRY
			...
			AND PayerItemDirtyFlag != @DebitDirtyFlag;
			...
			ELSE IF @ItemNotArchived > 0
					BEGIN
						...
						SET @Severity = 3;
						SET @Message = '[MOException] EOD- MO Internal ' + @DownstreamComponent + ' Reconciliation process for ' + @RoleFlag + ' ' + @Window + ' failed.' + CAST(@ItemNotArchived AS VARCHAR(10)) + ' Item(s) sent to SWITCH(MSG07) have not been ARCHIVED.';
			...
			ELSE IF @ReconItemCount > 0
					BEGIN
						...
						SET @Result = [$(MoConfigDatabase)].[config].[sfn_ReadScalarConfigValue]('EOD', 'FailureCode');
						SET @Severity = 3;
						SET @Message = '[MOException] EOD- MO Internal ' + @DownstreamComponent + ' Reconciliation process for ' + @RoleFlag + ' ' + @Window + ' failed.' + CAST(@ReconItemCount AS VARCHAR(10)) + ' Items(s) have not been sent to ARCHIVE for MSG09,MSG11,MSG12.';
						SET @SysOut = @ErrorCode + ':' + @Message;
					END;
