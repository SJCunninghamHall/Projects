    ...
    BEGIN
        ...
        BEGIN TRY
			...
			UNION ALL
				...
				CREATE CLUSTERED INDEX ci_ItemId ON #ItemsCTE(ItemId)
				CREATE NONCLUSTERED INDEX nci_Currency ON #ItemsCTE(Currency)
				CREATE NONCLUSTERED INDEX nci_ItemType ON #ItemsCTE(ItemType)
				CREATE NONCLUSTERED INDEX nci_InternalTxId ON #ItemsCTE(InternalTxId)
				CREATE NONCLUSTERED INDEX nci_SortCode ON #ItemsCTE(SortCode)
				CREATE NONCLUSTERED INDEX nci_UniqueItemIdentifier ON #ItemsCTE(UniqueItemIdentifier)
				CREATE NONCLUSTERED INDEX nci_CaptureItemId ON #ItemsCTE(CaptureItemId)
				CREATE NONCLUSTERED INDEX nci_UniqueItemIdentifier ON #ItemsCTE(UniqueItemIdentifier)
				...
				SELECT 
					...
					FinalItms.OnUs OnUsItemIndicator,
					CASE 
						...
						ELSE 0
					END RTPPaidIndicator,
					COALESCE(FinalItms.Represent, 0) AS RePresentedItemIndicator,
					FrdStsRslts.FraudResult FraudCheckResult,
					CASE 
						WHEN	(
									FinalItms.RepairedSortcode = 1
								OR 
									FinalItms.RepairedAccount = 1
								OR 
									FinalItms.IsAmountCorrected = 1
								OR 
									FinalItms.IsANCorrected = 1
								OR 
									FinalItms.IsSortCodeCorrected = 1
								OR 
									FinalItms.IsSerialCorrected = 1
								) 
						THEN 1
						ELSE 0
					END RepairedItemIndicator,
					CASE 
						WHEN	(   
									FinalItms.DefaultedSortcode = 1
								OR 
									FinalItms.DefaultedAccount = 1
								) 
						THEN 1
						ELSE 0
					END DefaultedItemIndicator,
					CASE 
						WHEN	(   
									FinalItms.SwitchedSortCode IS NOT NULL
								OR 
									FinalItms.SwitchedAccount IS NOT NULL
								) 
						THEN 1
						ELSE 0
					END SwitchedItemIndicator,
					CASE 
					...
					vwTx.CollectingParticipantId CollectingParticipantIdentifier,
					CASE 
					...
					vwTx.TransactionSetIdWithVersion TransactionSetId,
					vwDoc.DocumentMessageId AS DocumentSubmissionIdentifier,
					FinalItms.APGDIN CaptureDIN,
				...
				LEFT JOIN 
					Lookup.ItemType ItmTp WITH (SNAPSHOT) 
				ON 
					...
					(   
						...
						FROM   
							(   
								SELECT 
									en.EntityId,
									ener.EntityId ErrorEntityId,
									en.EntityIdentifier,
									en.EntityState,
									ROW_NUMBER() OVER (PARTITION BY en.EntityIdentifier ORDER BY en.Revision DESC) EntityErrorRank
								FROM   
									Base.Entity  en WITH (SNAPSHOT)
								LEFT JOIN 
									Base.EntityError ener WITH (SNAPSHOT) 
								ON 
				...
				LEFT JOIN 
					[ReferenceData].[RTPPaidEntityStates] RTPConfig WITH (SNAPSHOT) 
				ON 
