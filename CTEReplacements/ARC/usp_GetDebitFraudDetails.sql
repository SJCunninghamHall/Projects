                ...
                BEGIN
                    IF ( @BusinessDate IS NOT NULL )
						...
						INTO
							#fraudDebits
						FROM   
							Base.Debit d WITH (SNAPSHOT)
						INNER JOIN 
							Base.DebitFraudData dfd WITH (SNAPSHOT) 
						ON 
							dfd.ItemId = d.ItemId
						LEFT JOIN 
						   (
						...
						WHERE  
							...
							AND 
								d.DebitId = @UniqueItemIdentifier
							)
						OR 
							(   
								@UniqueItemIdentifier IS NULL
							AND 
								@BusinessDate IS NOT NULL
							AND 
								d.ItemId
							BETWEEN @BusinessDateFrom AND @BusinessDateTo
							)

					CREATE CLUSTERED INDEX ci_UniqueItemIdentifier ON #fraudDebits(UniqueItemIdentifier)
					...
					FROM 
						#fraudDebits fraud WITH (SNAPSHOT) 
					INNER JOIN 
...
GRANT EXECUTE ON [Web].[usp_GetDebitFraudDetails] TO WebItemRetrieval;
GO
