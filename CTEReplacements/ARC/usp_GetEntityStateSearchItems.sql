    ...
    BEGIN
            ...
            IF @RoleName IS NOT NULL
				...
				SELECT 
					...
					CASE WHEN FinDeb.PayDecision = 1 THEN 1 ELSE 2 END  AS RTPPaidIndicator,
				...
				INTO
					#FinalItemsOuter
				FROM 
					Base.FinalDebit FinDeb WITH (SNAPSHOT)
				INNER JOIN 
					(
						SELECT     
							A.EntityId,
						...
						JOIN
							(
								...
								JOIN 
									ReferenceData.EntityStateSearchTypes ESST WITH (SNAPSHOT) 
								ON 
									En.EntityState = ESST.EntityState 
								AND 
									ESST.SearchTypeId= @SearchType
								JOIN 
									Base.Entity FinalES WITH (SNAPSHOT) 
								ON 
				...
				AND 
					DB.ItemId = FinDeb.ItemId 
				INNER JOIN 
					Base.Core Core WITH (SNAPSHOT) 
				ON 
					Core.CoreId = EN.CoreId
				INNER JOIN 
					Base.vw_TXSet TXSet WITH (SNAPSHOT) 
				ON 
					TXSet.InternalTxId = FinDeb.InternalTxId
				INNER JOIN 
					Lookup.ItemType IT WITH (SNAPSHOT) 
				ON 
					IT.Id = FinDeb.ItemType
				WHERE 
					FinDeb.ItemId BETWEEN @BusinessDateFrom AND @BusinessDateTo 
				AND 
					(@SortCode IS NULL OR FinDeb.Sortcode = @SortCode) 
				AND 
					(@Account IS NULL OR FinDeb.AccountNumber = @Account) 
				AND 
					(@AmountFilter IS NULL OR FinDeb.Amount = @AmountFilter) 
				AND 
					(@TranCode IS NULL OR FinDeb.TranCode = @TranCode) 
				AND
					(@Serial IS NULL OR FinDeb.Serial = @Serial)

				CREATE CLUSTERED INDEX ci_UniqueItemIdentifier ON #FinalItemsOuter(UniqueItemIdentifier)
				CREATE NONCLUSTERED INDEX nci_CurrentEntityState ON #FinalItemsOuter(CurrentEntityState)
				...
				SELECT 
					...
					CASE WHEN FSR.FraudResult = 'Not Processed' THEN 'NP'
						 WHEN FSR.FraudResult = 'OK' THEN 'OK'
						 WHEN FSR.FraudResult = 'Suspect' THEN 'SUS'
						 WHEN FSR.FraudResult = 'Fraudulent' THEN 'FRD'							 
						 WHEN FSR.FraudResult = 'KG' THEN 'KG' 
				...
				INTO
					#FinalItms
				FROM 
					#FinalItemsOuter FinalItemsOuter WITH (SNAPSHOT)
				LEFT JOIN	
					(
						SELECT	
							FraudId,
				...
				INNER JOIN 
					[ReferenceData].[RTPPaidEntityStates] RTP WITH (SNAPSHOT) 
				ON 
					RTP.EntityState = FinalItemsOuter.CurrentEntityState 			


			CREATE CLUSTERED INDEX ci_UniqueItemIdentifier ON #FinalItms(UniqueItemIdentifier)
			CREATE NONCLUSTERED INDEX nci_Currency ON #FinalItms(Currency)
					...
					FROM 
						(
							SELECT
								...
								PD.ItemId AS ItemId,
								ROW_NUMBER() OVER (PARTITION BY PD.ItemId ORDER BY PD.InternalId DESC) AS RKD
							FROM 
								Post.PostingDebit PD WITH (SNAPSHOT)
							INNER JOIN 
								#FinalItms FinalItms
							ON 
			...
			LEFT JOIN 
				[Lookup].Currency CUR WITH (SNAPSHOT) 
			ON 
...
GRANT EXECUTE ON [Web].[usp_GetEntityStateSearchItems] TO WebItemRetrieval;
