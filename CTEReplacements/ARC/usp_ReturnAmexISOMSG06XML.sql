CREATE PROCEDURE [Agency].[usp_ReturnAmexISOMSG06XML]
...
/*****************************************************************************************************
* Name				: [Agency].[usp_ReturnAmexISOMSG06XML]]
* Description		: This Stored Procedure returns the XML for the Amex MSG06 ISO Extract 
		...
		SET NOCOUNT ON;
		 ...
		 BEGIN TRAN
			...
			DECLARE @EncodedSignature VARCHAR(MAX) = CAST(N'' as xml).value('xs:base64Binary(sql:variable("@InputSignature"))','VARCHAR(MAX)')
				...
				SELECT	ETSET.TransactionSetIdWithVersion
						,ETSET.ItemId
				...
				FROM
					(
						SELECT	
							...
							,0 AS Amt --CR.Amount AS Amt --(As per the Bug 212237 fix)
							,CCY.Currency AS AmtCcy
					...
					FROM 
						#ExtractDebit EDB
					INNER JOIN 
						[Base].[vw_TXSet] TX ON EDB.TransactionSetIdWithVersion = TX.TransactionSetIdWithVersion 
					INNER JOIN 
						Base.vw_FinalCredit CR ON TX.InternalTxId = CR.InternalTxId
					INNER JOIN 
						Lookup.ItemType ITMTYPE WITH (SNAPSHOT) ON ITMTYPE.Id = CR.ItemType
					INNER JOIN 
						Lookup.Currency CCY WITH (SNAPSHOT) ON CCY.Id = CR.Currency
					INNER JOIN 
						Base.CreditFraudData CRFRD WITH (SNAPSHOT) ON CRFRD.ItemId = CR.ItemId
					) ECR
				WHERE 
					ECR.RKD = 1
			...
			SELECT
				...
				SELECT 				
						...
						SELECT	MsgId
								...
								,Sgntr
						FROM ExtractGrpHdr
						CROSS JOIN (SELECT COUNT(1) NumberofTransactions FROM #ExtractTXSet) CNTTXN
...
GO
GRANT EXECUTE ON [Agency].[usp_ReturnAmexISOMSG06XML] TO AgencyISOMsgExtractor;
...
EXECUTE sp_addextendedproperty @name = N'Component', @value = N'STAR',
    ...
    @level1name = N'usp_ReturnAmexISOMSG06XML';


GO
EXECUTE sp_addextendedproperty @name = N'MS_Description',
    @value = N'This Stored Procedure extracts an Amex XML extract in MSG06 ISO format for a given AgencyId.',
    @level0type = N'SCHEMA', @level0name = N'Agency', @level1type = N'PROCEDURE',
    @level1name = N'usp_ReturnAmexISOMSG06XML';
