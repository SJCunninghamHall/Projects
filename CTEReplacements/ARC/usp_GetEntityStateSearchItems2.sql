		...
		BEGIN TRY
			...
			AS
				(
							...
							JOIN
								(
									...
									FROM
										[Base].[Entity]								AS [En]
									JOIN
										[ReferenceData].[EntityStateSearchTypes]	AS [ESST]
									ON
										[En].[EntityState] = [ESST].[EntityState]
									AND [ESST].[SearchTypeId] = @SearchType
									JOIN
										[Base].[Entity]								AS [FinalES]
									ON
					...
					INNER JOIN
						[Base].[vw_TXSet]	AS [TXSet]
					ON
						[TXSet].[InternalTxId] = [FinDeb].[InternalTxId]
					INNER JOIN
						[Lookup].[ItemType] AS [IT]
					ON
			...
			AS
				(
					...
					INNER JOIN
						[ReferenceData].[RTPPaidEntityStates]	AS [RTP]
					ON
