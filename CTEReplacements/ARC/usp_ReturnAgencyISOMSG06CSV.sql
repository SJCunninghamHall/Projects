...
AS
		...
		SET TRANSACTION ISOLATION LEVEL READ COMMITTED;
		 ...
		 BEGIN TRY
			...
			SELECT	
				...
				,ROW_NUMBER() OVER (ORDER BY EligibleTransactionSets.ItemId ASC) AS	TransactionSetRank
			FROM 
			(
				SELECT	
					...
					,DB.ItemId
					,TX.InternalTxId
					,ROW_NUMBER() OVER (PARTITION BY FD.DebitId ORDER BY FD.ItemId DESC) RKD
				FROM 
				...
				ON 
					IU.CoreID = CO.CoreId
				INNER JOIN 
					Lookup.MessageType MSGTYPE WITH (SNAPSHOT) 
				ON 
				...
				AND 
					FD.ItemId BETWEEN @BusinessDateFrom	AND @BusinessDateTo
	...
	BEGIN
		SELECT Result.RowValue
		  FROM (
				 SELECT RowValue = 'Item ID,Sortcode,Account,Serial,Amount,Kappa Flag,T-set Identifier,Collecting Participant,Switched Sortcode,Switched Account,Debit Item Type,Debit Item Transaction Code,Represented Item Indicator' ,
                        1 AS RNO
                  UNION ALL
	...
	BEGIN
		...
		INNER JOIN 
			[Base].[vw_TXSet] TX ON ETSET.InternalTxId = TX.InternalTxId	
		INNER JOIN 
			Base.Debit DB WITH (SNAPSHOT) ON DB.ItemId = ETSET.ItemId 
		INNER JOIN 
			Base.vw_FinalDebit FD ON DB.DebitId=FD.DebitId
		INNER JOIN 
			Base.ItemUpdate IU WITH (SNAPSHOT) ON IU.InternalId = DB.ItemId
		INNER JOIN 
			Base.Core CO WITH (SNAPSHOT) ON CO.CoreId = IU.CoreID
		INNER JOIN 
			Lookup.ItemType ITMTYPE WITH (SNAPSHOT) ON ITMTYPE.Id = FD.ItemType
		LEFT JOIN 
			Base.DebitFraudData	DBFRD WITH (SNAPSHOT) ON DBFRD.ItemId = DB.ItemId
		WHERE 
		...
		FROM 
			(
				SELECT 
					RowValue = 'Item ID,Sortcode,Account,Serial,Amount,Kappa Flag,T-set Identifier,Collecting Participant,Switched Sortcode,Switched Account,Debit Item Type,Debit Item Transaction Code,Represented Item Indicator' ,
					1 AS RNO
...
GO
GRANT EXECUTE ON [Agency].[usp_ReturnAgencyISOMSG06CSV] TO AgencyISOMsgExtractor;
