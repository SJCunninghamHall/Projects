...
AS
    ...
    DECLARE @ProcedureName sysname, @ErrorMessage varchar(4000),
	@Day2Date BIGINT
 
    SELECT @ProcedureName = OBJECT_NAME(@@PROCID), @ErrorMessage = NULL

	BEGIN TRY
 
			DELETE FROM [StarGalaxyExport].[ExportDay2XMLMessageIds];
			...
			SET @Day2Date =	(
								...
								FROM 
									Base.Entity E
								RIGHT JOIN 
									Base.Core C ON E.CoreId = C.CoreId
								INNER JOIN 
									Base.XMLMessage X ON X.Id = C.XMLMessageId
								WHERE 
									(
										EntityIdentifier IN	(
																SELECT 
																...
																ON 
																	C.CoreId = E.CoreId
																WHERE 
																	C.CoreId BETWEEN @LongTermArchiveMigrationDateRangeStart AND @LongTermArchiveMigrationDateRangeEnd
															)
								AND 
									LEFT(X.Id, 8) > LEFT(@LongTermArchiveMigrationDateRangeEnd, 8)
								)
								ORDER BY 
									LEFT(X.Id, 8)
							);
			...
			CREATE CLUSTERED INDEX ci_EntityIdentifier ON #Day1Entities(EntityIdentifier)
			...
			INSERT INTO 
				[StarGalaxyExport].[ExportDay2XMLMessageIds](XmlId)
			SELECT DISTINCT 
				X.Id as 'Xml ID' 
			FROM 
				Base.Entity E
			...
			(
				...
				OR 
					C.IntMessageType = 'PTMA01'  
				OR 
					(
						EntityType <> 'I'
					AND 
						C.IntMessageType = '07MA01'
					) 
				OR 
					(
						EntityType <> 'I'  
    ...
    BEGIN CATCH
        ...
        BEGIN
            SET @ErrorMessage =
                'Error!! Stored Procedure ' + @@SERVERNAME + '.' + DB_NAME() + '.' + @ProcedureName 
                + ' Returned the following error :- ' + CAST(ERROR_MESSAGE() as varchar(1000)) 
                + ' Line Number :- ' + CAST(ERROR_LINE()as varchar(1000)) 
        END

		EXECUTE [Base].[usp_LogAndRaiseError] @ErrorMessage;
	...
	Grant Execute ON [StarGalaxyExport].[usp_Export_StarGalaxyExport_Day2XMLMessageIDs] to StarGalaxyExporter
	GO
